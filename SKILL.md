...
**Execution environment check — before any operation:** Lectic requires filesystem access, Python 3.10+, command execution and persistent project storage. Verify the current host's actual tools, interpreter version and access to the selected project before capturing or processing content. Use existing verified session evidence; otherwise perform a minimal command and temporary file write/read check in the authorized project, removing only that probe afterward. A chat attachment, browser tool or temporary code sandbox alone does not establish access to the user's persistent project.

If ChatGPT lacks these capabilities, explain that executing Lectic requires Work mode with the necessary project/command access. Hand off there using an available supported handoff, or give a concise continuation containing the user's exact links, context, goal and project location when switching must be done by the user. Do not claim a transfer occurred without confirmation, assume Work mode grants access, or begin a workflow that cannot be completed in the current environment. Recheck capabilities after switching. Codex and Claude Code hosts with the required access can run Lectic directly. Preserve existing saved work and resume it rather than recapturing it.
...
For “save this link,” share/capture requests, or receiving source content (links, videos, pasted text, transcripts, files) without an explicit collection named, **always ask which collection to add it to before assuming or creating a new one.** Users are normally adding to an existing collection and need to see their existing collections to choose from. Check existing collections and identify intelligent candidate matches (using topic, creator, and keyword matching via `candidate_collections` or `lectic_collection_candidates`), present the candidate existing collections alongside the list of all collections, and clarify if they would prefer to create a new collection instead. Only once confirmed (or if explicitly named upfront), save it to that collection. Follow [capture.md](prompts/capture.md) first. Capture means cheap storage, not permission to extract, discover or compile. Personal notes remain separate from sources. If a later goal uses a capture-fed collection, process available pending material when useful and disclose unavailable linked content. The [iPhone Shortcut proof](docs/iphone-shortcut.md) is the first folder-based adapter, not an installed native app.
...
If content arrives without a goal, first ask which collection to add it to (showing candidate and existing collections or offering to create a new one). Once saved, do not require the user to invent a goal: if they want to explore what the collection can become, produce a Capability Map following [opportunity-discovery.md](prompts/opportunity-discovery.md). Also use that flow for “What can I build?”, “What should I build first?”, or “Show me what this collection is capable of.” “Build #2” or a named opportunity continues directly from the saved map. “Just save for later” archives without extraction. “Save these as Leadership” prepares knowledge without forcing a map or result.
