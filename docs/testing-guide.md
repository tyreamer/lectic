...
For the capture-first pilot, use the [iPhone Shortcut live test](iphone-shortcut.md#exact-first-live-test-from-your-iphone). It separately measures saving, sync and import before asking for processing. A URL-only capture initially awaits retrieval; requested processing can acquire YouTube English captions when optional yt-dlp is available. Other linked platforms remain unavailable for content-based work, which is different from a failed save. Use the [YouTube regression/live retest](YOUTUBE.md#regression-and-live-retest) to assess the link-to-outcome journey without manually preparing transcripts.
...
Choose one project folder and keep using it throughout the test. Start with a small collection, such as 2–5 transcripts or YouTube videos, so you can judge whether the assistant interpreted them correctly. This is a suggested pilot size, not a product limit. Supply UTF-8 `.txt`, `.md`, `.vtt` or `.srt` files, or paste YouTube links and request processing. YouTube requires optional yt-dlp; include its setup time and retrieval failures in your feedback. No raw audio/video transcription, PDF/slide interpretation or arbitrary URL fetching is supplied. Do not hide a broken YouTube journey by having the facilitator download transcripts manually.
...
Give 2–4 participants an optional capture track across the existing groups. Let them save a few items encountered naturally, including a YouTube link, an unsupported-platform URL and actual shared text. Record save success, sync delay, import retries, repeated-import duplicates, annotation handling and a later real use. Don't hand-curate captures into transcripts. Separately record missing dependency, unavailable captions, access/network failure and unsupported platforms. A successful save is not a successful retrieval, and downloaded captions are not a useful outcome.

### YouTube user-journey regression

Use the eight real URLs and private entrepreneurship context in `fixtures/youtube/entrepreneurship.json`; start with a small subset for live acquisition. Test both “Save these for later” (no retrieval) and a subsequent “Use this collection to help me become an entrepreneur.” The assistant should retrieve captions itself, preserve all exact links including timestamps, explain unavailable items, and continue through source-backed preparation to a useful outcome. Personal aspirations and income claims must not become source facts or guarantees.

Inspect actual raw caption files, metadata and cited segments. Include a failed link beside a working one and confirm the available material still proceeds. Retry processing, then reopen the same project in a fresh session: successful captions and validated knowledge should be reused, with prior results unchanged. Record host, yt-dlp version, videos successfully acquired, caption type, time spent, errors and usefulness. Automated tests mock captions and use authored semantic artifacts; they do not establish live availability, assistant follow-through or entrepreneurship effectiveness.
